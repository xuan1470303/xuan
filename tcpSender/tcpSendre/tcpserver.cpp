    ...
    : QDialog(parent)
{
    ...
    connect(&tcp_Server, SIGNAL(acceptError(QAbstractSocket::SocketError)),this,
...
void tcpServer::start()
{
    ...
    while(!tcp_Server.isListening() &&
           !tcp_Server.listen(QHostAddress::AnyIPv4, 16998))
    {
        QMessageBox::StandardButton ret = QMessageBox::critical(this,
                                                                QStringLiteral("迴圈"),
                                                                QStringLiteral("無法啟動伺服器: %1.").arg(tcp_Server.errorString()),
...
{
    tcpServerConnection = tcp_Server.nextPendingConnection();//取得已接受的客戶端連線
...
void tcpServer::displayError(QAbstractSocket::SocketError socketError)
{
    ...
    if (server == &tcp_Server) qDebug() << "Hi I am QTcpServer";
